<nav class="bg-white shadow">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden md:flex md:items-center md:space-x-4">
                ...
                <div class="relative group/sub">
                    <button class="w-full text-left px-4 py-2 hover:bg-gray-100 inline-flex items-center justify-between">
                        Services
                        <svg class="w-4 h-4 transform transition-transform" id="products-arrow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                    <div class="absolute left-full top-0 hidden group-hover/sub:block w-48 bg-white shadow-lg py-2">
                        @foreach($categories as $id => $category)
                            <a href="/job/create?category={{$category->name}}" class="block px-3 py-2 hover:bg-gray-100">{{$category->name}}</a>
                        @endforeach
                    </div>
                </div>
    ...
    <div id="mobile-menu" class="hidden md:hidden">
        <div class="px-2 pt-2 pb-3 space-y-1">
            ...
            <div class="relative">
                <button onclick="toggleSubmenu('products-submenu')" class="w-full text-left px-3 py-2 hover:bg-gray-100 inline-flex items-center justify-between">
                    Services
                    <svg class="w-4 h-4 transform transition-transform" id="products-arrow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                    </svg>
